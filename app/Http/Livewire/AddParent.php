...
class AddParent extends Component
{
    ...
    public $currentStep = 1,
        ...
        protected $rules = [
            ...
            'Passport_ID_Father' => 'required|min:10|max:10',
            'Phone_Father' => 'required|string|min:10|max:15|regex:/[0-9]{9}/',
            'National_ID_Mother' => 'required|string|min:10|max:10|regex:/[0-9]{9}/',
            'Passport_ID_Mother' => 'min:10|max:10',
            'Phone_Mother' => 'required|string|min:10|max:15|regex:/[0-9]{9}/',
        ];
    ...
    public function submitForm(){
        ...
        try {
            ...
            $this->successMessage = trans('massages.success');
            $this->clearForm();
            $this->currentStep = 1;
        }
    ...
    public function submitForm_edit(){
        ...
        if ($this->Parent_id){
            ...
            //update National_ID_Father
            if($parent->National_ID_Father != $this->National_ID_Father){
                $pathold=storage_path('app/parent_attachments/') . $parent->National_ID_Father;
                $pathnew=storage_path('app/parent_attachments/') . $this->National_ID_Father;
                $attachments=ParentAttachment::where('parent_id',$this->Parent_id)->get();
                foreach($attachments as $attach){
                    if(!File::exists($pathnew)) {
                        File::makeDirectory($pathnew);
                    }
                    File::move($pathold.'/'.$attach->file_name,$pathnew.'/'.$attach->file_name );
                }
                ...
                $parent->update([ 'National_ID_Father' => $this->National_ID_Father]);


            }

        }
        $this->successMessage = trans('massages.update');
        return redirect()->to('/Add_parent');
    }

    public function delete($id){
       //delete attachments
        if(ParentAttachment::where('parent_id',$id)->first()->exists()){
            $attachments=ParentAttachment::where('parent_id',$id)->get();
            foreach($attachments as $attach){
                $attach->forceDelete();
            }
            $Details=My_Parent::find($id);
            File::deleteDirectory(storage_path('app/parent_attachments/') . $Details->National_ID_Father);
        }
        //delete parent
